...
do_encode({memo_device_info, _, _, _, _, _, _, _} =
	      Query,
	  TopXMLNS) ->
...
do_get_ns({memo_device_info, _, _, _, _, _, _, _}) ->
    <<"jabber:memo:device">>.

pp(memo_device_info, 7) ->
    [user, server, device_type, token, language,
     device_number, domain_key];
pp(_, _) -> no.

records() -> [{memo_device_info, 7}].
...
decode_memo_device_info(__TopXMLNS, __Opts,
    ...
    {memo_device_info, User, Server, Device_type, Token,
     Language, Device_number, Domain_key}.
