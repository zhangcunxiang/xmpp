...
pp(query_topic_info, 7) ->
    [tid, tname, tcreater, tcreate_time, topic_type,
     user_item, tmaxnum];
pp(mod_topic, 5) ->
    [rtype, code, tuser, topic_info, user_topic_list];
pp(_, _) -> no.
...
decode_mod_topic_els(__TopXMLNS, __Opts, [],
		     User_topic_list, Topic_info) ->
    {lists:reverse(User_topic_list), Topic_info};
decode_mod_topic_els(__TopXMLNS, __Opts,
...
encode_mod_topic({mod_topic, Rtype, Code, Tuser,
		  Topic_info, User_topic_list},
		 __TopXMLNS) ->
    __NewTopXMLNS =
	xmpp_codec:choose_top_xmlns(<<"jabber:iq:topic">>, [],
				    __TopXMLNS),
    _els =
	lists:reverse('encode_mod_topic_$user_topic_list'(User_topic_list,
							  __NewTopXMLNS,
							  'encode_mod_topic_$topic_info'(Topic_info,
											 __NewTopXMLNS,
											 []))),
...
'encode_mod_topic_$user_topic_list'([], __TopXMLNS,
				    _acc) ->
    _acc;
'encode_mod_topic_$user_topic_list'([User_topic_list
				     | _els],
				    __TopXMLNS, _acc) ->
    'encode_mod_topic_$user_topic_list'(_els, __TopXMLNS,
					[encode_query_topic_info(User_topic_list,
								 __TopXMLNS)
					 | _acc]).
